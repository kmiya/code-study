use crate::InterpreterErrorKind::DivisionByZero;
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        ...
        match self {
            ...
            NotOperator(tok) => write!(f, "{}: {} is not an open operator", tok.loc, tok.value),
    ...
    Tokens: Iterator<Item = Token>,
{
    match tokens.peek().map(|tok| tok.value) {
        Some(TokenKind::Plus) | Some(TokenKind::Minus) => {
            ...
            // , ATOM
            let exp = parse_atom(tokens)?;
            let loc = op.loc.merge(&exp.loc);
            Ok(Ast::uniop(op, exp, loc))
        }
        // | ATM
        _ => parse_atom(tokens),
    }
...
where
    Tokens: Iterator<Item = Token>,
{
    let mut exp = subexpr_parser(tokens)?;
    loop {
        match tokens.peek() {
            Some(_) => {
                ...
                let loc = exp.loc.merge(&r.loc);
                exp = Ast::binop(op, exp, r, loc)
            }
            _ => break,
        }
    }
